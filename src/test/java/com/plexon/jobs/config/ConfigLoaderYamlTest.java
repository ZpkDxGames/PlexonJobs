...
class ConfigLoaderYamlTest {
    ...
    @Test void malformedCandidateYamlFailsClosed() throws Exception {
        ...
        IllegalArgumentException failure = assertThrows(IllegalArgumentException.class,
                () -> ConfigLoader.validateYaml(config.toFile(), "config.yml"));
    ...
    @Test void validCandidateYamlPassesStrictValidation() throws Exception {
        Path config = temp.resolve("config.yml");
        Files.writeString(config, "runtime:\n  mode: PRIMARY\n");

        assertDoesNotThrow(() -> ConfigLoader.validateYaml(config.toFile(), "config.yml"));
    }
}
