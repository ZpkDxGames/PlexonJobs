...
/**
 * Async persistence boundary for DailyLimitService. Gameplay calls ensure(...) and proceeds only
 * when it returns true; SQL never runs on the caller/gameplay thread.
 */
public final class DailyLimitPersistence {
    ...
    public DailyLimitPersistence(PlexonCoreAPI core, JobsDatabase database,
                                 DailyLimitService limits, Logger logger) {
        this.core = core;
    ...
    /**
     * Starts one asynchronous hydration when needed. False means gameplay must fail closed until a
     * later call observes the completed authoritative same-day snapshot.
     */
    public boolean ensure(UUID playerId) {
        ...
        try {
            CompletableFuture<Map<String, JobsDatabase.DailyRow>> future =
                    core.scheduler().supplyIo(() -> database.loadDaily(playerId, dayId));
            ...
            future.whenComplete((rows, error) -> core.scheduler().runPrimary(() -> {
                ...
                Map<String, DailyLimitService.CounterView> restored = rows.entrySet().stream()
                        .collect(java.util.stream.Collectors.toUnmodifiableMap(
                                Map.Entry::getKey,
                                entry -> new DailyLimitService.CounterView(
                                        entry.getValue().moneyMinor(), entry.getValue().xp())));
    ...
    public void flushDirty() {
        for (UUID playerId : limits.dirtyPlayers()) {
            if (loaded.contains(playerId)) saveAsync(playerId);
        }
    }
    ...
    private void saveAsync(UUID playerId) {
        ...
        DailyLimitService.Snapshot snapshot = limits.snapshot(playerId);
        try {
            CompletableFuture<Void> future = core.scheduler().runIo(
                    () -> database.saveDaily(playerId, snapshot.dayId(), databaseRows(snapshot)));
            ...
            future.whenComplete((unused, error) -> core.scheduler().runPrimary(() -> {
                ...
                if (error != null) {
                    logger.log(Level.SEVERE, "Failed to persist daily Jobs state for " + playerId, error);
                } else {
                    limits.markPersisted(playerId, snapshot.revision());
                }
            }));
    ...
    private static Map<String, JobsDatabase.DailyRow> databaseRows(DailyLimitService.Snapshot snapshot) {
        return snapshot.rows().entrySet().stream().collect(java.util.stream.Collectors.toUnmodifiableMap(
                Map.Entry::getKey,
                entry -> new JobsDatabase.DailyRow(entry.getValue().moneyMinor(), entry.getValue().xp())));
    }
}
